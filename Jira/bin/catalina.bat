...
if not exist "%CATALINA_BASE%\bin\check-java.bat" goto checkJavaHome
    call "%CATALINA_BASE%\bin\check-java"
    if errorlevel 1 exit /b 1
    goto checkJavaDone
:checkJavaHome
if not exist "%CATALINA_HOME%\bin\check-java.bat" goto checkJavaDone
    call "%CATALINA_HOME%\bin\check-java"
    if errorlevel 1 exit /b 1
:checkJavaDone

if exist "%CATALINA_BASE%\bin\set-gc-params.bat" (
    call "%CATALINA_BASE%\bin\set-gc-params"
    goto setGcParamsDone
)
if exist "%CATALINA_HOME%\bin\set-gc-params.bat" (
    call "%CATALINA_HOME%\bin\set-gc-params"
    goto setGcParamsDone
)
:setGcParamsDone

if exist "%CATALINA_BASE%\bin\set-opens.bat" (
    call "%CATALINA_BASE%\bin\set-opens"
    goto setOpensDone
)
if exist "%CATALINA_HOME%\bin\set-opens.bat" (
    call "%CATALINA_HOME%\bin\set-opens"
    goto setOpensDone
)
:setOpensDone
