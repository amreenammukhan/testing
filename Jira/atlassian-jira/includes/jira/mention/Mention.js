define('jira/mention/mention', ['jira/util/key-code', 'jira/ajs/control', 'jira/dialog/dialog-stack', 'jira/mention/mention-user', 'jira/mention/mention-group', 'jira/mention/mention-matcher', 'jira/mention/scroll-pusher', 'jira/mention/contextual-mention-analytics-event', 'jira/mention/uncomplicated-inline-layer', 'jira/ajs/layer/inline-layer/standard-positioning', 'jira/ajs/layer/inline-layer/window-positioning', 'jira/ajs/dropdown/dropdown-list-item', 'jira/util/events', 'aui/progressive-data-set', 'jquery', 'underscore', 'wrm/context-path', 'jira/util/logger', 'jira/util/formatter'], function (keyCodes, Control, DialogStack, UserModel, MentionGroup, MentionMatcher, jiraScrollPusher, ContextualMentionAnalyticsEvent, UncomplicatedInlineLayer, InlineLayerStandardPositioning, InlineLayerWindowPositioning, DropdownListItem, Events, ProgressiveDataSet, jQuery, _, contextPath, logger, formatter) {
    ...
    var mentionsStatusTimeoutId = undefined;
    var shouldUpdateResults = false;
    var shouldAnnounceDropdown = false;
    ...
    return Control.extend({
        ...
        updateSuggestions: function updateSuggestions($suggestions) {
            ...
            if (this.layerController) {
                requestAnimationFrame(function () {
                    ...
                    _this2.updateMentionsStatus(shouldUpdateResults);
                });
            }
        },
        ...
        /**
         * Updates content of the assistive container for screen reader.
         * @param {boolean} areResultsUpdated If set to true, it will update status with results amount and after fixed timeout.
         */
        updateMentionsStatus: function updateMentionsStatus(areResultsUpdated) {
            clearTimeout(mentionsStatusTimeoutId);
            var instance = this;
            mentionsStatusTimeoutId = setTimeout(function () {
                mentionsStatusTimeoutId = undefined;

                var dropdownExpandedInformation = shouldAnnounceDropdown ? formatter.I18n.getText('jira.mentions.dropdown.announcement') : '';
                var resultsAmount = instance.listController.items ? instance.listController.items.length : 0;

                var results = '';
                if (areResultsUpdated) {
                    results = resultsAmount > 0 ? formatter.I18n.getText('jira.mentions.results.amount', resultsAmount) : formatter.I18n.getText('jira.mentions.nomatch', instance.currentUserName, '', '');
                }

                var selectedUserName = resultsAmount > 0 ? formatter.I18n.getText('jira.mentions.selected.user', instance.getSelectedUserName()) : '';

                if (instance.layerController.isVisible()) {
                    var assistiveHelper = instance.layerController.layer().find('div.assistive[role="status"]');
                    assistiveHelper.html((dropdownExpandedInformation + ' ' + results + ' ' + selectedUserName).trim());
                    shouldAnnounceDropdown = false;
                }
            }, !areResultsUpdated ? 350 : 1000);
        },

        getSelectedUserName: function getSelectedUserName() {
            return idOf((this.listController.highlighted || this.listController.items[this.listController.index] || {}).$element);
        },
        ...
        _keyUp: function _keyUp() {
            ...
            var areMentionsUpdated = false;
            shouldAnnounceDropdown = !mentioning || shouldAnnounceDropdown;
            shouldUpdateResults = this.currentUserName !== this.lastQuery || shouldAnnounceDropdown;
            if (this._isNewRequestRequired(username)) {
                this.fetchUserNames(username);
                areMentionsUpdated = true;
                mentioning = true;
            ...
            if (!areMentionsUpdated && mentioning) {
                this.updateMentionsStatus(false);
            }
