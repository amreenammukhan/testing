define('jira/ajs/list/list', ['jira/util/logger', 'jira/util/formatter', 'jira/util/strings', 'jira/ajs/control', 'jira/ajs/list/group-descriptor', 'jira/ajs/list/item-descriptor', 'jira/ajs/input/mouse', 'jira/util/assistive', 'jira/util/browser', 'jquery', 'underscore'], function (logger, formatter, strings, Control, GroupDescriptor, ItemDescriptor, Mouse, Assistive, Browser, jQuery, _) {
    ...
    function buildGroupHeaderId(id) {
        return buildElementId(id) + "-header";
    }

    function buildGroupDescriptionId(id) {
        return buildElementId(id) + "-description";
    }
    ...
    return Control.extend({
        ...
        _getDefaultOptions: function _getDefaultOptions() {
            return {
                ...
                suggestionsListRole: "listbox",
                rootRole: "presentation",
                subtextPrefix: "",
                loopThroughOptions: false,
                prioritizeExactMatches: false, // Whether to move exact matches to the top of the list
                hideSuggestionCount: false,
                listId: ""
            };
        },
        ...
        moveToNext: function moveToNext() {
            if (this.index < this.maxIndex) {
                ...
                ++this.index; // increase the index
                this.focus(); // focus it
            } else if (this.$visibleItems.length > 1 && this.options.loopThroughOptions) {
                this.index = 0;
                this.focus();
            }
        },
        ...
        moveToPrevious: function moveToPrevious() {
            if (this.index > 0) {
                ...
                this.focus();
            } else if (this.$visibleItems.length > 0 && this.options.loopThroughOptions) {
        ...
        _addTitlesToProjectItems: function _addTitlesToProjectItems(data) {
            function addTitleIfAbsent(item) {
                if (item.properties.title === undefined) {
                    item.properties.title = item.properties.label;
                }
            }

            data.forEach(function (descriptor) {
                if (descriptor instanceof GroupDescriptor) {
                    descriptor.properties.items.forEach(function (item) {
                        addTitleIfAbsent(item);
                    });
                } else if (descriptor instanceof ItemDescriptor) {
                    addTitleIfAbsent(descriptor);
                }
            });
        },
        ...
        _makeResultDiv: function _makeResultDiv(data, query) {
            var $fixedContainer = jQuery('<div class="aui-list-fixed" role="' + this.options.rootRole + '">');
            var $resultsContainer = jQuery('<div class="aui-list-scroll" tabindex="-1" role="' + this.options.rootRole + '" id="' + this.options.listId + '">');
            var instance = this;
            var ungrouped = [];

            if (this.$container && this.$container.attr('id') === 'project-suggestions') {
                this._addTitlesToProjectItems(data);
            }
        ...
        _makeSuggestionFooter: function _makeSuggestionFooter(footerMessage) {
            return jQuery('<li class="no-suggestions">' + footerMessage + "</li>");
        },
        ...
        _addOptionsToContainer: function _addOptionsToContainer(options, $container, query, labelRegex) {
            ...
            for (var i = 0, len = options.length; i < len; i++) {
                ...
                if (i < maxResult || forceAllResults) {
                    ...
                    var limitedSuggestionsMessage = this._createLimitedSuggestionsMessage(instance, options.length - i);
                    suggestions.push(limitedSuggestionsMessage);
        ...
        _createLimitedSuggestionsMessage: function _createLimitedSuggestionsMessage(instance, optionsCount) {
            this.hasTooManySuggestions = true;
            var $tooMany = instance._render("tooManySuggestions", optionsCount);
        ...
        _filterAndAddOptions: function _filterAndAddOptions(options, container, query) {
            ...
            if (query) {
                ...
                if (this.options.prioritizeExactMatches) {
                    options = this._moveExactMatchesToTop(options, query);
                }
            }
        ...
        /**
         * Move exact matches to the beginning of the results so they are visible first
         * @param {ItemDescriptor[]} options - The list of filtered items
         * @param {string} query - The search query
         * @returns {ItemDescriptor[]} - The same filtered list of items but with exact matches placed at the beginning
         */
        _moveExactMatchesToTop: function _moveExactMatchesToTop(options, query) {
            var nonExactMatches = [];
            var exactMatches = options.filter(function (option) {
                if (option.label().toLowerCase() === query.toLowerCase()) {
                    return true;
                } else {
                    nonExactMatches.push(option);
                    return false;
                }
            });

            return exactMatches.concat(nonExactMatches);
        },
        ...
        _renders: {
            ...
            suggestion: function suggestion(descriptor, replacementText) {
                ...
                var listElem = jQuery('<li class="aui-list-item"><' + this.options.listItemTag + ' class="aui-list-item-link" role="none" aria-selected="false" /></li>').attr({
                ...
                if (descriptor.labelSuffix()) {
                    var suffixSpan = jQuery("<span class='aui-item-suffix' />").text(this.options.subtextPrefix + descriptor.labelSuffix());
                    linkElem.wrapInner("<span class='aui-item-text' />").append(suffixSpan);
                }

                if (descriptor.icon() && descriptor.icon() !== "none") {
                    var icon = jQuery('<img class="icon"  alt="" loading="lazy" />');
                    icon.attr("src", descriptor.icon());
                    if (descriptor.iconType() === 'rounded') {
                        icon.addClass('rounded');
                    }
            ...
            tooManySuggestions: function tooManySuggestions(suggestionCount) {
                if (this.options.expandAllResults) {
                    ...
                    $button = null;
                    return $li;
                } else if (this.options.hideSuggestionCount) {
                    return this._makeSuggestionFooter(formatter.I18n.getText("common.concepts.continue.typing.to.refine.further"));
                } else {
                    return this._makeSuggestionFooter(formatter.I18n.getText("common.concepts.too.many.matches", suggestionCount));
                }
            },
            ungroupedSuggestions: function ungroupedSuggestions() {
                return jQuery('<ul>').attr("role", this.options.suggestionsListRole);
            },
            suggestionGroup: function suggestionGroup(descriptor) {
                var list = jQuery("<ul/>").attr({
                    "class": "aui-list-section",
                    "role": this.options.suggestionsListRole
                }).addClass(descriptor.styleClass()).data("descriptor", descriptor);

                var label = descriptor.label();
                if (label) {
                    list.attr({
                        "id": buildElementId(label),
                        "aria-labelledby": buildGroupHeaderId(label)
                    });

                    var description = descriptor.description();
                    if (description) {
                        list.attr("aria-describedby", buildGroupDescriptionId(label));
                    }
                }
                return list;
            },
            suggestionGroupHeading: function suggestionGroupHeading(descriptor) {
                var label = descriptor.label();
                var elem = jQuery("<h5 />").text(label).addClass(descriptor.styleClass()).data("descriptor", descriptor).attr({
                    "id": buildGroupHeaderId(label),
                    "aria-hidden": true
                });

                if (descriptor.description()) {
                    jQuery("<span class='aui-section-description' />").text(" (" + descriptor.description() + ")").attr({
                        "id": buildGroupDescriptionId(label),
                        "aria-hidden": true
                    }).appendTo(elem);
                }
