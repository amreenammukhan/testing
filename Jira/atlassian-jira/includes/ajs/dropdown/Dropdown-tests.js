AJS.test.require(["jira.webresources:key-commands", "jira.webresources:dropdown"], function () {
    ...
    test("Pressing enter on a trigger should focus the dropdown content container when opened", function () {
        trigger.appendTo(document.body);
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger
        ...
        equal($("#dropDown").get(0) === document.activeElement, true, "focus should be on the dropdown content container");
    });
    ...
    test("All the links have tabindex='0' for them to be focusable even if they don't have href", function () {
        dropDown = $("<div id='dropDown' class='aui-list'>" + "<ul id='list_id'>" + "<li id='list-entry-1'>" + "<a id='test-link-1' href='/test/path/1'>Test 1</a>" + "</li>" + "<li id='list-entry-2'>" + "<a id='test-link-2'>Test 2</a>" + "</li>" + "</ul>" + "</div>");
        ...
        var containsNonFocusableLinks = false;
        $("#dropDown a").each(function () {
            if ($(this).attr("tabindex") !== "0") {
                containsNonFocusableLinks = true;
            }
        });

        equal(containsNonFocusableLinks, false, "all the links are focusable");
    });
    ...
    module("Dropdown events propagation", {
        setup: function setup() {
            clickPropagated = false;
            escapePropagated = false;

            this.clock = sinon.useFakeTimers();
            trigger = $("<a id='trigger' href='#whoops'>Trigger</a>").appendTo(document.body);
            dropDown = $("<div id='dropDown' class='aui-list'>" + "<ul id='list_id'>" + "<li><a id='test-link-1' href='/test/path/1'>Test 1</a></li>" + "<li><a id='test-link-2' href='/test/path/2'>Test 2</a></li>" + "<li><a id='test-link-3' href='/test/path/3'>Test 3</a></li>" + "</ul>" + "</div>");
    ...
    test("Click event on trigger is not propagated if stopTriggerClickPropagation is set to true", function () {
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger,
            stopTriggerClickPropagation: true
        });

        click(trigger);
        this.clock.tick(4);

        equal(clickPropagated, false, "Click event is not propagated");
    });

    test("Click event on trigger is propagated if stopTriggerClickPropagation is set to false", function () {
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger,
        ...
        equal(clickPropagated, true, "Click event is propagated");
    });

    test("Click event on trigger is propagated if stopTriggerClickPropagation is not set", function () {
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger
        });
    ...
    test("Escape keydown event on content is not propagated if stopEscapePropagation is set to true", function () {
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger,
        ...
        click(trigger);
        dropDown.trigger($.Event("keydown", { key: "Escape" }));
        this.clock.tick(4);

        equal(escapePropagated, false, "Escape keydown event is not propagated");
    });

    test("Escape keydown event on content is propagated if stopEscapePropagation is set to false", function () {
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger,
        ...
        equal(escapePropagated, true, "Escape keydown event is propagated");
    });

    test("Escape keydown event on content is not propagated if stopEscapePropagation is not set", function () {
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger
        });
    ...
    test("Create dropdown in same DOM element as Trigger element, instead of at the end of body", function () {
        AJS.Dropdown.create({
            content: dropDown,
            trigger: trigger,
            appendDropdownToTrigger: true
        });
        click(trigger);
        equal(dropDown.parentNode === trigger.parentNode, true, "Dropdown is appended in same DOM element as Trigger element");
    });
});