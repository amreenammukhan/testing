define('jira/ajs/dropdown/dropdown', ['jira/ajs/control', 'jira/ajs/layer/inline-layer', 'jira/ajs/layer/layer-constants', 'jira/ajs/layer/layer-interactions', 'jira/ajs/layer/hide-reasons', 'jira/ajs/dropdown/dropdown-constants', 'jira/ajs/dropdown/dropdown-list-item', 'jira/ajs/dropdown/dropdown-options-descriptor', 'underscore', 'jquery'], function (Control, InlineLayer, LayerConstants, interactions, LayerHideReason, Constants, ListItem, OptionsDescriptor, _, $) {
    ...
    var Dropdown = Control.extend({
        ...
        _createFocusHandler: function _createFocusHandler($content) {
            $content.css({ outline: 0 }).attr("tabindex", -1);
            return function (e) {
                if (e.key !== "Tab") {
                    return;
                }
                var focusableElements = $content.find(":focusable");
        ...
        init: function init(options) {
            ...
            if (!layerProperties.appendToTrigger) {
                layerProperties.appendToTrigger = this.options.appendDropdownToTrigger();
            }
            ...
            this.layerController.contentChange(function () {
                ...
                var $els = instance.layerController.layer().find("div > ul[role=menu] > li > ul[role=group] > li:visible:has(a)");
                if (!$els.length) {
                    $els = instance.layerController.layer().find("div > ul > li:visible:has(a)");
                }
            ...
            this.content($content); // bind content events
        ...
        show: function show() {
            ...
            if (this.content()) {
                this._handleTabPress = this._createFocusHandler(this.content());
                this.content().bind("keydown", this._handleTabPress);
            }
        ...
        hide: function hide() {
            ...
            if (this.content()) {
                this.content().unbind("keydown", this._handleTabPress);
            }
        ...
        content: function content(_content) {
            if (_content) {
                if (this.options.content()) {
                    this._unassignEvents("content", this.options.content());
                }
                this._assignEvents("content", this.options.content());
        ...
        _events: {
            trigger: {
                "aui:keydown": function auiKeydown(e) {
                    ...
                    if (e.key === "Down" && !this.layerController.isVisible()) {
                        e.preventDefault();
                        this.show();
                    }
                },
                click: function click(e) {
                    ...
                    this.options.stopTriggerClickPropagation() && e.stopPropagation();
                    this.toggle();
                }
            },
            content: {
                keydown: function keydown(e) {
                    if (e.key === "Escape" && this.options.stopEscapePropagation()) {
                        e.stopPropagation();
                        this.hide();
                        this.trigger().focus();
                    }
