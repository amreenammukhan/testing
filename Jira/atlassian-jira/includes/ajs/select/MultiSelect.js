define('jira/ajs/select/multi-select', ['jira/util/formatter', 'jira/ajs/layer/layer-constants', 'jira/ajs/select/multi-select/lozenge', 'jira/ajs/select/multi-select/lozenge-group', 'jira/ajs/select/queryable-dropdown-select', 'jira/ajs/select/select-helper', 'jira/ajs/select/select-model', 'jira/ajs/select/suggestions/select-suggest-handler', 'jira/ajs/layer/inline-layer-factory', 'jira/ajs/list/list', 'jira/ajs/list/item-descriptor', 'jira/util/objects', 'jira/util/assistive', 'jquery'], function (formatter, LayerConstants, MultiSelectLozenge, MultiSelectLozengeGroup, QueryableDropdownSelect, SelectHelper, SelectModel, SelectSuggestHandler, InlineLayerFactory, List, ItemDescriptor, Objects, Assistive, jQuery) {
    'use strict';
    ...
    return QueryableDropdownSelect.extend({
         ...
         * @param {String} [options.ariaLabel] - Aria-label for the form element.
         * @param {String} [options.ariaLabelledBy] - Aria-labelledby for the form element.
         * @param {Object} [options.events] - Event groups to be merged with defaults and attached to the component's furniture
        ...
        _createFurniture: function _createFurniture(disabled) {
            ...
            var ariaDescribedBy = this.model.$element.attr("aria-describedby");
            var isInvalid = this.model.$element.attr("aria-invalid");
            ...
            if (disabled) {
                ...
                this.$field = this._render("field", id, ariaDescribedBy, isInvalid).appendTo(this.$container);

                this.$label = jQuery('label[for="' + jQuery.escapeSelector(id) + '"]');
                if (this.$label.length) {
                    this.$label.attr('for', this.$field.attr('id'));
                } else {
                    this.$label = jQuery('label[for="' + jQuery.escapeSelector(this.$field.attr('id')) + '"]');
                }

                if (this.$field.attr('role') === 'combobox') {
                    if (ariaDescribedBy) {
                        this.ariaDescriptionForCombobox = jQuery('#' + jQuery.escapeSelector(ariaDescribedBy)).text();
                    }

                    if (!this.$field.attr('aria-label')) {
                        this.$field.attr('aria-label', [this.$label.text(), this.ariaDescriptionForCombobox].filter(function (element) {
                            return element;
                        }).join('. ') || null);
                    }
                }
        ...
        /**
         * Extend events by merging events passed in a constructor
         * @private
         */
        _prepareEvents: function _prepareEvents() {
            Object.assign(this._events, this.options.events);
        },
        ...
        _assignEventsToFurniture: function _assignEventsToFurniture() {
            ...
            this._assignEvents("element", this.model.$element);
        ...
        updateAriaLabel: function updateAriaLabel() {
            if (this.ariaLabelTimerDebounce) {
                window.clearTimeout(this.ariaLabelTimerDebounce);
            }

            var updateLabel = function () {
                var labelText = this.$label.text();
                var descriptionText = this.ariaDescriptionForCombobox || '';
                var selectedValues = this.model.getDisplayableSelectedDescriptors().map(function (option) {
                    return option.label();
                });
                var selectedValuesText = selectedValues.length > 0 ? selectedValues.join(", ") : '';
                var shouldPlaceAriaLabel = !!(labelText || selectedValuesText || descriptionText);
                this.$field.attr("aria-label", shouldPlaceAriaLabel ? labelText + '- ' + selectedValuesText + '. ' + descriptionText : null);
            }.bind(this);

            this.ariaLabelTimerDebounce = window.setTimeout(updateLabel, 100);
        },
        ...
        /**
         * In some context (e.g. issue create/edit form) indentation needed to make space for lozenges in input is calculated too early.
         * This method returns true only if there's very high probability that indentation was not calculated properly. Two conditions are checked:
         * 1) is thery any lozenge?
         * 2) if so - is left padding bigger than the default one
         *
         * @return boolean
         */
        _probablyNeedsIndentationSet: function _probablyNeedsIndentationSet() {
            if (this.lozengeGroup.items.length === 0) {
                return false;
            }

            var defaultPaddingLeft = 8;
            var currentPaddingLeft = parseInt(this.$field.css('paddingLeft'));

            if (currentPaddingLeft > defaultPaddingLeft) {
                return false;
            }
        ...
        _getInputIndent: function _getInputIndent() {
            ...
            if (lastLozengeIndex >= 0) {
                ...
                top = $last.prop("offsetTop") - paddingTop;

                if (this.$container && this.$container.closest(".inline-edit-fields").size()) {
                    top += 3;
                }

                left = $last.prop("offsetLeft") + $last.outerWidth() + paddingLeft;
        ...
        _selectionHandler: function _selectionHandler(selected, e) {
            ...
            this.hideAriaInvalid();
            this.hideAriaDescribedBy();
            this.updateFreeInputVal();
        ...
        _events: {
            ...
            field: {
                ...
                focus: function focus() {
                    if (this._probablyNeedsIndentationSet()) {
                        this.updateItemsIndent();
                    }
                }
            },
        ...
        _renders: {
            ...
            field: function field(idPrefix, ariaDescribedBy, isInvalid) {
                //  the wrap="off" attribute prevents text from growing under the labels. It doesn't prevent linebreaks
                var $field = this._render("baseField", "textarea").attr({
                ...
                if (ariaDescribedBy) {
                    $field.attr("aria-describedby", ariaDescribedBy);
                }
                if (isInvalid) {
                    $field.attr("aria-invalid", isInvalid);
                }
                if (this.options.ariaLabel) {
                    $field.attr("aria-label", this.options.ariaLabel);
                }
                if (this.options.ariaLabelledBy) {
                    $field.attr("aria-labelledby", this.options.ariaLabelledBy);
                }
