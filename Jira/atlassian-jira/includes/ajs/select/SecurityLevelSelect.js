define('jira/ajs/select/security-level-select', ['jira/util/key-code', 'jira/util/formatter', 'jira/util/strings', 'jira/analytics', 'jira/issue', 'jira/featureflags/feature-manager', 'jira/ajs/select/security/default-comment-security-level-control', 'jira/ajs/select/security/select-adapter', 'jira/ajs/select/dropdown-select', 'jira/ajs/layer/inline-layer/window-positioning', 'jquery'], function (keyCodes, formatter, StringUtils, analytics, Issue, DarkFeatures, DefaultCommentSecurityLevelControl, SecuritySelectAdapter, DropdownSelect, WindowPositioning, jQuery) {
    ...
    return DropdownSelect.extend({
        ...
        init: function init(commentLevelSelect) {
            ...
            windowPositioning.setInversePositioningIfNotFit(true);

            this._super(commentLevelSelect, { setMaxHeightToWindow: true, positioningController: windowPositioning });
        ...
        _handleDownKey: function _handleDownKey(e) {
            if (this.dropdownController.isVisible()) {
                if (e.keyCode === keyCodes.UP && e.altKey) {
                    e.preventDefault();
