    ...
    <ui:soy moduleKey="'com.atlassian.auiplugin:soy'" template="'aui.page.pagePanel'">
        <ui:param name="'content'">
            ...
            <form id="issuepicker-source" name="selectFilter" class="aui top-label" action="IssuePicker.jspa">
                <div class="group" aria-label="<ww:text name="'issuepicker.radio.filter.aria.label'" />">
                    <div class="radio">
                        <input type="radio" id="issue-source-recent" class="radio" name="mode" value="recent" <ww:if test="mode == 'recent'">checked aria-checked="true"</ww:if><ww:if test="mode != 'recent'">aria-checked="false"</ww:if> aria-label="<ww:text name="'issuepicker.recent.issues'" />"/>
                        <label for="issue-source-recent" title="<ww:text name="'issuepicker.recent.issues.desc'" />">
                            <ww:text name="'issuepicker.recent.issues'" />
                        </label>
                    </div>
                    <div class="radio">
                        <input type="radio" id="issue-source-search" class="radio" name="mode" value="search" <ww:if test="mode == 'search'">checked aria-checked="true"</ww:if><ww:if test="mode != 'search'">aria-checked="false"</ww:if> aria-label="<ww:text name="'issuepicker.search.filter'" />"/>
                ...
                <div class="field-group">
                    <label for="searchRequestId"><ww:text name="'issuepicker.select.filter.label'"/></label>
                    <select id="searchRequestId" name="searchRequestId" onChange="submit()" class="select">
                        <option value="-1" aria-selected="true"><ww:text name="'issuepicker.select.value'"/></option>
                        <ww:iterator value="/availableFilters">
                            <option value="<ww:property value="./id"/>"<ww:if test="./id == /searchRequestId && mode=='search'">selected aria-selected="true"</ww:if><ww:if test="./id != /searchRequestId || mode!='search'">aria-selected="false"</ww:if>><ww:property value="./name"/></option>
                        </ww:iterator>
                    </select>
                </div>
                <div class="hidden">
