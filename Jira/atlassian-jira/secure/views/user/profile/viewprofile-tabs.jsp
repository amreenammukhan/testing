                        ...
                        <ul class="aui-nav" id="user_profile_tabs">
                            <ww:iterator value="/tabDescriptors" status="'status'">
                                <li id="up_<ww:property value="./key"/>_li" class="<ww:if test="/selected == completeKey">aui-nav-selected</ww:if><ww:if test="@status/first == true"> first</ww:if>">
                                    <a class="aui-nav-item" id="up_<ww:property value="./key"/>_a" aria-current="page" href='<%= request.getContextPath() %>/secure/ViewProfile.jspa?<ww:if test="/user != /loggedInUser">name=<ww:property value="user/name"/>&</ww:if>selectedTab=<ww:property value="./completeKey"/>' title="<ww:text name="./name"/>"><ww:text name="./name"/></a>
                                </li>
                            </ww:iterator>
                        </ul>
                    </div>
